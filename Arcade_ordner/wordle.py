        ...
        if len(guess) == len(word):
            ...
            for index in range(word_size):
                ...
                if word[index] == guess[index]:
                    word_interface[lig][index] = CGREENBG + BLACK + " " + guess[index] + " " + RESET + GREY
            ...
            for index1 in range(word_size):
                if guess[index1] in word and guess[index1] in more_letters and not word[index1] == guess[index1]:
                    word_interface[lig][index1] = CYELLOWBG + BLACK + " " + guess[index1] + " " + RESET + GREY
                    more_letters.remove(guess[index1])
                elif not word[index1] == guess[index1]:
                    word_interface[lig][index1] = GREY + " " + guess[index1] + " "
            lig += 1
            guessing = wordle_win_loose_detection(lig, word_size, word, guess, word_interface)
        else:
...
def wordle_word_size(language):
    ...
    request = True
    while request:
        user_input = input(BLUE + "Select the number of letters in the word (3 to 6): ")
        try:
            ...
            if 3 <= int(user_input) <= 6:
                if user_input == 3:
                    if language == "German":
                        words = ["auf", "aus", "das", "ufo", "ehe", "gab", "fee", "tee", "zug"]
                        break
                    elif language == "English":
                        words = ["bit", "gap", "gas", "guy", "bro", "dog", "cat", "hut", "mad"]
                        break
                elif user_input == 4:
                    if language == "German":
                        words = ["bahn", "auto", "sehr", "fein", "hund", "jahr", "jagd", "gans"]
                        break
                    elif language == "English":
                        words = ["area", "army", "baby", "card", "fish", "news", "wall", "bear"]
                        break
                elif user_input == 5:
                    if language == "German":
                        words = ["hagel", "haase", "fahne", "tiere", "haare", "sache", "fahrt"]
                        break
                    elif language == "English":
                        words = ["anger", "apple", "beach", "drink", "dress", "hotel", "steam"]
                        break
                elif user_input == 6:
                    if language == "German":
                        words = ["abflug", "achsel", "zahlen", "machen", "jaguar", "radler"]
                        break
                    elif language == "English":
                        words = ["better", "bottle", "damage", "eleven", "family", "garden"]
                        break
            else:
...
def wordle_word_list():
    ...
    while user_input != 1 and user_input != 2:
        user_input = input(MAGENTA + "Please select the language of the words: ")
        if user_input == "1":
...
def wordle_game():
    ...
    while game:
        ...
        while again != "y" and again != "n":
            print(BLUE + f"Wanna play again? {len(words)} words left to guess!")
